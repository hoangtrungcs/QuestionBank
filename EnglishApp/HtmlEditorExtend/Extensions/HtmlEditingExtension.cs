...
namespace HtmlEditorExtend.Extensions
{
    internal static class HtmlEditingExtension
    {
        ...
        /// <summary>
        /// Trung:Insert seperate at cursor posion and format the seperate that you want.
        /// </summary>
        public static void InsertSeperate(this HtmlDocument document, SeperateObject seperate)
        {
            var txt = seperate.Text.HtmlEncoding();
            if (string.IsNullOrEmpty(txt)) txt = Constants.QuestionKeyForBlank;
            string tx = string.Format("<font color=blue>{0}</font> ", txt);
